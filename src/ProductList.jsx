...
function ProductList() {
    ...
    const [showPlants, setShowPlants] = useState(false); // State to control the visibility of the About Us pagetot
    const [addToCart,setAddTocart]=useState({})
     // Calculer le nombre total d'articles dans le panier
     const cartItems = useSelector(state => state.cart.items);

     // Calculer le nombre total d'articles dans le panier
     const totalItems = cartItems.reduce((total, item) => total + item.quantity, 0);  // Additionner les quantités
    ...
    const badgeStyle = {
        position: 'absolute',
        top: '5px',  // Ajuster la position selon vos préférences
        right: '5px',
        backgroundColor: 'red',
        color: 'white',
        borderRadius: '50%',
...
const handleContinueShopping = () => {
    setShowCart(false);  // Remet showCart à false pour afficher à nouveau les produits
  };
         ...
         {plantsArray.map((category,index)=>(
            <div key={index}>
                ...
                <div className='product-list'>
                    {category.plants.map((plant,indexp)=>(
                        <div key={indexp} className='product-card'>
